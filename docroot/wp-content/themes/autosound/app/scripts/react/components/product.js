...
export default class Product extends Component {
  ...
  render() {
    ...
    if (product.isEmpty()) {
      return <NotFound title="Not Found" subtitle="The product you are looking for does not exist." />;
    }
          ...
          <div className="shop-product__inner">
            {product.get('images') && product.get('images').count() ? (
              <div className="shop-product__image">
                <ProductCarousel images={product.get('images')} />
              </div>
            ) : null}
            <div className="shop-product__content">
              ...
              <div dangerouslySetInnerHTML={innerHtml(product.get('description'))} className="shop-product__description" />
              ...
              {product.getIn(['buy', 'link']) ? (
                <a href={product.getIn(['buy', 'link'])} className="btn btn-primary btn-sm" style={{marginRight: 5}}>
                  {product.getIn(['buy', 'text']) && product.getIn(['buy', 'text']) !== '' ? product.getIn(['buy', 'text']) : 'Buy Now'}
                </a>
              ) : null}
              <a href={wordpressConstants.customQuotePage} className="btn btn-primary btn-sm">
                Custom Quote
              </a>
            </div>
          </div>
        ...
        {featuredProducts && featuredProducts.count() ? (
          <div className="related-products">
            <h3 className="related-products__title">
              Related <span className="is-red">Products</span>
            </h3>
            <ShopGrid
              defaultCount={3}
